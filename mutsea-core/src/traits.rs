...
use crate::{MutseaResult, UserId, RegionId, AssetId, ObjectId};
...
pub trait UserService: Service {
    ...
    async fn authenticate(&self, first_name: &str, last_name: &str, password: &str) -> MutseaResult<Option<UserId>>;
    
    /// Create a new user account
    async fn create_user(&self, first_name: &str, last_name: &str, email: Option<&str>, password: &str) -> MutseaResult<UserId>;
...
pub trait RegionService: Service {
    ...
    async fn get_regions_by_location(&self, x_min: u32, y_min: u32, x_max: u32, y_max: u32) -> MutseaResult<Vec<crate::RegionInfo>>;
}
...
pub trait DatabaseService: Service {
    ...
    async fn query_one<T>(&self, query: &str, params: &[&dyn std::fmt::Debug]) -> MutseaResult<Option<T>>
    where
        T: for<'de> Deserialize<'de> + Send + Sync;
